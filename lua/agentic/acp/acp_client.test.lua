...
describe("ACPClient", function()
    ...
    describe("permission callback with nil option_id", function()
        --- @type TestStub
        local with_subscriber_stub

        before_each(function()
            -- Stub __with_subscriber to call the callback synchronously
            -- This avoids vim.schedule making assertions unreachable in same-process tests
            ---@diagnostic disable-next-line: invisible
            with_subscriber_stub = spy.stub(ACPClient, "__with_subscriber")
            with_subscriber_stub:invokes(function(self, session_id, callback)
        ...
        after_each(function()
            with_subscriber_stub:revert()
        end)

        it("sends cancelled outcome instead of selected with nil", function()
            local client = create_ready_client()

            --- @type fun(option_id: string|nil)|nil
            local captured_permission_callback

            --- @type agentic.acp.ClientHandlers
            local handlers = {
                on_session_update = function() end,
                on_request_permission = function(_request, callback)
                    captured_permission_callback = callback
                end,
                on_error = function() end,
                on_tool_call = function() end,
                on_tool_call_update = function() end,
            ...
            -- Call __handle_request_permission directly to exercise permission logic
            ---@diagnostic disable-next-line: invisible
            client:__handle_request_permission(99, {
                sessionId = "session-1",
                toolCall = {
                    toolCallId = "tc-1",
                    kind = "edit",
            ...
            -- Synchronous due to stubbed __with_subscriber
            assert.is_not_nil(captured_permission_callback)

            -- Reset send tracking so we can capture the result response
            transport_send_stub:reset()

            -- Call callback with nil (simulating cancel/stop_generation)
            --- @cast captured_permission_callback fun(option_id: string|nil)
            captured_permission_callback(nil)

            -- Should have sent one result message
            assert.spy(transport_send_stub).was.called(1)

            local sent_data = transport_send_stub.calls[1][2]
            local decoded = vim.json.decode(sent_data)

            -- Must be a JSON-RPC result (not request)
            assert.equal(99, decoded.id)
            assert.is_nil(decoded.method)

            -- outcome must be "cancelled", NOT "selected" with nil optionId
            assert.equal("cancelled", decoded.result.outcome.outcome)
            assert.is_nil(decoded.result.outcome.optionId)
        end)

        it("sends selected outcome with valid option_id", function()
            local client = create_ready_client()
            ...
            ---@diagnostic disable-next-line: invisible
            client:__handle_request_permission(100, {
                sessionId = "session-1",
                toolCall = {
                    toolCallId = "tc-2",
                    kind = "edit",
            ...
            --- @cast captured_permission_callback fun(option_id: string|nil)
            captured_permission_callback("allow")
            ...
            assert.equal(100, decoded.id)
            assert.equal("selected", decoded.result.outcome.outcome)
            assert.equal("allow", decoded.result.outcome.optionId)
        end)
    end)
end)
