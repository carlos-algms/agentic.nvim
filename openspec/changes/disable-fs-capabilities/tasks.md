...
- [ ] 1.1 Write tests: `_handle_notification` logs warning
  (not processes) for `fs/read_text_file` and
  `fs/write_text_file`
- [ ] 1.2 Replace `fs/read_text_file` and `fs/write_text_file`
...
- [ ] 1.3 Remove `_handle_read_text_file` method
  (`acp_client.lua:471-490`)
- [ ] 1.4 Remove `_handle_write_text_file` method
  (`acp_client.lua:494-511`)
- [ ] 1.5 Remove `FileSystem` require from `acp_client.lua`
  (no longer used there)
- [ ] 1.6 Add `write` to `KNOWN_ACP_KINDS` in `acp_client.lua`
...
- [ ] 2.1 Write tests for force-reload behavior:
...
- [ ] 2.2 Register a `FileChangedShell` autocommand that sets
...
- [ ] 3.1 Write tests for buffer reload behavior:
...
- [ ] 3.2 Add constant set of file-mutating tool call kinds:
  `edit`, `create`, `write`, `delete`, `move`
- [ ] 3.3 In `on_tool_call_update` handler, look up `kind` from
  `message_writer.tool_call_blocks[tool_call_id]` with nil
  check on both tracker and `kind` field
- [ ] 3.4 When `status == "completed"` and kind is file-mutating,
  call `vim.cmd.checktime()`

## 4. Validation

- [ ] 4.1 Run `make validate` — all checks must pass
