...
- [x] 1.1 Create `message_writer.test.lua` co-located test file with
...
- [x] 1.3 Test: returns `false` when cursor is far from buffer end
  (user scrolled up)
- [x] 1.4 Test: returns `false` when threshold is 0 or nil (disabled)
- [x] 1.5 Test: returns `true` when window is not visible (winid=-1)
...
- [x] 4.1 Run `make validate` and fix any issues
